...
# new route for reset page ,  needs a secure token
@auth.route("/reset_token", methods={"GET","POST"})
...
if new_pass == confirm_pass:
        ...
        # Use the CRUD update method to change the password
        updated_user = CRUD.update(User, user.user_id, password=new_pass) 
        return jsonify({'message': 'Password reset link sent '})
        # redirect to login page?
else:
    return jsonify({'Passwords do not match. Please try again'})
    # redirect to /reset_token so can try again
